...
data "archive_file" "cloudDeployPubSub" {
  type = "zip"
  output_path = "/tmp/function-cloudDeployPubSub.zip"
  source_dir = "CloudFunctions/cloudDeployPubSub/"
}
...
resource "google_storage_bucket_object" "cloudDeployPubSub" {
  name = "function-cloudDeployPubSub.zip"
  bucket = google_storage_bucket.function_bucket.name
  source = data.archive_file.cloudDeployPubSub.output_path
}
...
# Create a Cloud Function to interact with Cloud Deploy
resource "google_cloudfunctions2_function" "cloudDeployPubSub" {
  name    = "cloud-deploy-pubsub"
  project = var.project_id
  location = var.region

  build_config {
    entry_point = "cloudDeployPubSub"
    runtime     = "go122" # Or your preferred runtime
    source {
      storage_source {
        bucket = google_storage_bucket.function_bucket.name # Replace with your bucket name
        object = google_storage_bucket_object.cloudDeployPubSub.name # Replace with your source code object
      }
    }
  }
